        ...
        <nav class="mobile-menu popup__mobile-menu">
          <ul class="mobile-menu__ul">
            <li class="mobile-menu__li">
              <a class="mobile-menu__link popup-close" href="#" data-scroll-to="section-catalog">Піцца</a>
            </li>
            <li class="mobile-menu__li">
              <a class="mobile-menu__link popup-close" href="#" data-scroll-to="section-about">Про нас</a>
            </li>
            <li class="mobile-menu__li">
              <a class="mobile-menu__link popup-close" href="#" data-scroll-to="section-contacts">Контакти</a>
            </li>
          </ul>
        </nav>
        <div class="phone popup__phone">
          <a class="phone__item phone__item--accent" href="tel:+79999999999">+380 (999) 999-99-99</a>
        </div>
        <ul class="socials">
  ...
  <div class="popup__wrapper">
    <div class="popup__inner">
      <div class="popup__content">
        <button class="btn-close popup__btn-close popup-close"></button>
        <h2 class="page-title popup__title">Заповніть форму</h2>
        <p class="popup__subtitle popup__subtitle--order">Ми привеземо вам гарячу піцу за 50 хвилин</p>
        <div class="order">
          ...
          <form class="form form-send">
            <input class="order-info-title" type="hidden" name="Піцца">
            <input class="order-info-price" type="hidden" name="Ціна">
            <input class="order-info-size" type="hidden" name="Розмір">
            <input class="form__input" type="text" name="имя" placeholder="Імя" required>
            <input class="form__input" type="text" name="телефон" placeholder="Телефон" required>
            <input class="form__input" type="text" name="адрес" placeholder="Адреса" required>
            <select class="form__input" name="оплата">
              <option value="Оплата після доставки">Оплата після доставки</option>
...
<div class="popup popup-thanks">
  <div class="popup__wrapper">
    <div class="popup__inner">
      <div class="popup__content">
        <button class="btn-close popup__btn-close popup-close"></button>
        <h2 class="page-title popup__title">Успішно відправлено</h2>
        <p class="popup__subtitle">Ми вже почали готувати вашу піцу</p>
      </div>
    </div>
  </div>
</div>
...
<!-- /.popup-error -->
  ...
  <?php
require 'vendor/autoload.php'; // Підключення бібліотеки Twilio

use Twilio\Rest\Client;

if($_SERVER['REQUEST_METOD'] == 'POST'){
  $name = $_POST["Діма"];
  $phone = $_POST["+380973144326"];
  $address = $_POST["Головна"];
  $payment = $_POST["Готівка"];

// Twilo Account SID та Auth Token
  $sid = 'ACf9c6b4eb042121f436f34392dbe21b76';
  $token = '8d3f0602462f9f8bba34b6b538cfbc62';


  //Номер з якого буде відправлятись SMS Взяти із Twilo
  $fromNumber = '+1 484 388 5292';
  ...
  //Відповідь на клієнтську сторону
    echo "success";

} else{
  //Не POST запит ми ігноруємо
